...
const stateBackupService_1 = require("../services/stateBackupService");
// ── Mock helpers ──────────────────────────────────────────────
...
function createService(initialState = {}) {
    ...
    const svc = new stateBackupService_1.StateBackupService(ctx, out);
    return { svc, ctx, out };
}
// ── Tests ─────────────────────────────────────────────────────
...
async function testSnapshotExcludesBackupKey() {
    ...
    assert.strictEqual(entry.snapshot['stellarSuite.stateBackups'], undefined, 'snapshot should not include the backup storage key');
...
async function testRestoreFromBackup() {
    ...
    assert.deepStrictEqual(ctx.workspaceState.get('stellarSuite.configurations', {}), { network: 'mainnet' });
    // Restore
    ...
    assert.deepStrictEqual(ctx.workspaceState.get('stellarSuite.configurations', {}), { network: 'testnet' });
...
async function testRestoreFromCorruptedBackup() {
    ...
    const entries = ctx.workspaceState.get('stellarSuite.stateBackups', []);
...
async function testImportBackupsRejectsInvalidFormat() {
    ...
    try {
        ...
        threw = true;
        assert.ok(e.message.includes('entries'));
    }
...
async function testRestorePreservesBackupEntries() {
    ...
    assert.strictEqual(backupCountAfter, backupCountBefore, 'backup entries should be preserved after restore');
