...
const parameterValidator_1 = require("../services/parameterValidator");
const environmentValidator_1 = require("../services/environmentValidator");
const fileValidator_1 = require("../services/fileValidator");
const preFlightCheckService_1 = require("../services/preFlightCheckService");
const cliValidationService_1 = require("../services/cliValidationService");
...
test('validateParameter: required parameter present passes', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, 'some-value');
    assert.strictEqual(result.valid, true);
    assert.strictEqual(result.errors.length, 0);
});
test('validateParameter: required parameter missing fails', async () => {
    const schema = { name: 'id', type: 'string', required: true };
    const result = (0, parameterValidator_1.validateParameter)(schema, undefined);
...
test('validateParameter: required parameter empty string fails', async () => {
    const schema = { name: 'id', type: 'string', required: true };
    const result = (0, parameterValidator_1.validateParameter)(schema, '   ');
    assert.strictEqual(result.valid, false);
    assert.strictEqual(result.errors[0].code, 'MISSING_PARAMETER');
});
test('validateParameter: optional parameter missing passes', async () => {
    const schema = { name: 'label', type: 'string', required: false };
    const result = (0, parameterValidator_1.validateParameter)(schema, undefined);
    assert.strictEqual(result.valid, true);
});
test('validateParameter: string format valid matches pattern', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, validId);
    assert.strictEqual(result.valid, true);
});
test('validateParameter: string format invalid fails with pattern', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, 'invalid-id');
    assert.strictEqual(result.valid, false);
...
test('validateParameter: number type valid integer', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, 42);
    assert.strictEqual(result.valid, true);
});
...
test('validateParameter: number type invalid string fails', async () => {
    const schema = { name: 'count', type: 'number', required: true };
    const result = (0, parameterValidator_1.validateParameter)(schema, 'not-a-number');
    assert.strictEqual(result.valid, false);
    assert.strictEqual(result.errors[0].code, 'INVALID_TYPE');
});
test('validateParameter: number below min fails', async () => {
    const schema = { name: 'port', type: 'number', required: true, min: 1 };
    const result = (0, parameterValidator_1.validateParameter)(schema, 0);
    assert.strictEqual(result.valid, false);
    assert.strictEqual(result.errors[0].code, 'OUT_OF_RANGE');
});
test('validateParameter: number above max fails', async () => {
    const schema = { name: 'port', type: 'number', required: true, max: 65535 };
    const result = (0, parameterValidator_1.validateParameter)(schema, 70000);
    assert.strictEqual(result.valid, false);
    assert.strictEqual(result.errors[0].code, 'OUT_OF_RANGE');
});
test('validateParameter: number within range passes', async () => {
    const schema = { name: 'port', type: 'number', required: true, min: 1, max: 65535 };
    const result = (0, parameterValidator_1.validateParameter)(schema, 8080);
    assert.strictEqual(result.valid, true);
});
test('validateParameter: boolean true passes', async () => {
    const schema = { name: 'verbose', type: 'boolean', required: false };
    const result = (0, parameterValidator_1.validateParameter)(schema, true);
    assert.strictEqual(result.valid, true);
});
test('validateParameter: boolean string "yes" passes', async () => {
    const schema = { name: 'verbose', type: 'boolean', required: false };
    const result = (0, parameterValidator_1.validateParameter)(schema, 'yes');
    assert.strictEqual(result.valid, true);
});
test('validateParameter: boolean invalid string fails', async () => {
    const schema = { name: 'verbose', type: 'boolean', required: false };
    const result = (0, parameterValidator_1.validateParameter)(schema, 'maybe');
    assert.strictEqual(result.valid, false);
...
test('validateParameter: enum valid value passes', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, 'testnet');
    assert.strictEqual(result.valid, true);
});
...
test('validateParameter: enum invalid value fails', async () => {
    ...
    const result = (0, parameterValidator_1.validateParameter)(schema, 'devnet');
    assert.strictEqual(result.valid, false);
...
test('validateCommandParameters: valid parameters pass', async () => {
    ...
    const result = (0, parameterValidator_1.validateCommandParameters)(schema, {
        '--network': 'testnet',
        '--source': 'dev',
    }, silentLogger);
    assert.strictEqual(result.valid, true);
...
test('validateEnvironment: no requirements passes', async () => {
    const result = (0, environmentValidator_1.validateEnvironment)({ logger: silentLogger });
    assert.strictEqual(result.valid, true);
});
test('validateEnvironment: missing env var fails', async () => {
    const varName = 'STELLAR_TEST_VAR_THAT_DOES_NOT_EXIST_' + Date.now();
    const result = (0, environmentValidator_1.validateEnvironment)({
...
test('validateFilePath: existing file passes', async () => {
    ...
    const result = (0, fileValidator_1.validateFilePath)({
        filePath: __filename,
        expectedType: 'file',
    }, silentLogger);
    assert.strictEqual(result.valid, true);
...
test('validateFilePaths: multiple files validated', async () => {
    ...
    const result = (0, fileValidator_1.validateFilePaths)(rules, silentLogger);
    assert.strictEqual(result.valid, false);
...
test('CommandSyntaxCheck: valid params passes', async () => {
    const check = new preFlightCheckService_1.CommandSyntaxCheck();
...
test('CliAvailabilityCheck: skipped when not required', async () => {
    const check = new preFlightCheckService_1.CliAvailabilityCheck();
    const result = await check.execute({
...
test('EnvironmentCheck: skipped when no requirements', async () => {
    const check = new preFlightCheckService_1.EnvironmentCheck();
    const result = await check.execute({
...
test('FileValidationCheck: skipped when no rules', async () => {
    const check = new preFlightCheckService_1.FileValidationCheck([]);
    const result = await check.execute({
        commandSchema: { name: 'test' },
        parameters: {},
        cliPath: 'stellar',
...
test('NetworkConnectivityCheck: skipped when not required', async () => {
    const check = new preFlightCheckService_1.NetworkConnectivityCheck();
    const result = await check.execute({
...
test('runPreFlightChecks: build command with no CLI passes syntax', async () => {
    const report = await (0, preFlightCheckService_1.runPreFlightChecks)({
        commandSchema: {
            name: 'build',
            requiresCli: false,
...
test('validateCommand: unknown command fails', async () => {
    const result = await (0, cliValidationService_1.validateCommand)({
...
test('formatValidationResult: passed output contains checkmark', async () => {
    ...
    const formatted = (0, cliValidationService_1.formatValidationResult)(result);
...
test('formatPreFlightReport: contains check labels', async () => {
    ...
    const formatted = (0, preFlightCheckService_1.formatPreFlightReport)(report);
...
test('COMMAND_SCHEMAS: deploy schema exists and requires network', async () => {
    const schema = cliValidationService_1.COMMAND_SCHEMAS['deploy'];
    assert.ok(schema);
...
test('COMMAND_SCHEMAS: build schema exists and does not require network', async () => {
    const schema = cliValidationService_1.COMMAND_SCHEMAS['build'];
    assert.ok(schema);
    assert.strictEqual(schema.requiresNetwork, false);
    assert.strictEqual(schema.requiresCli, true);
});
test('COMMAND_SCHEMAS: simulate schema has contract ID validation', async () => {
    const schema = cliValidationService_1.COMMAND_SCHEMAS['simulate'];
    assert.ok(schema);
...
test('generateUsageString: produces correct format', async () => {
    ...
    const usage = (0, parameterValidator_1.generateUsageString)(schema);
