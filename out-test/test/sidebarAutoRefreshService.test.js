...
const sidebarAutoRefreshService_1 = require("../services/sidebarAutoRefreshService");
function createService(overrides = {}) {
    ...
    const outputChannel = {
        appendLine: (_msg) => undefined,
    };
    const service = new sidebarAutoRefreshService_1.SidebarAutoRefreshService(sidebarProvider, outputChannel, {
...
async function testRelevantPathFiltering() {
    ...
    assert.strictEqual(service.isRelevantPath('/workspace/contracts/alpha/target/wasm32-unknown-unknown/release/alpha.wasm'), true);
    service.dispose();
...
async function testDebounceBatchesRapidChanges() {
    ...
    assert.deepStrictEqual((refreshCalls[0].changedPaths || []).sort(), ['/workspace/contracts/a/Cargo.toml', '/workspace/contracts/b/Cargo.toml'].sort());
    service.dispose();
