...
exports.ReorderingService = void 0;
...
class ReorderingService {
     ...
     */
    getOrder() {
        return this.context.workspaceState.get(ORDER_KEY, []);
    }
    /**
     * Persist a new order. Validates that paths are non-empty strings.
     */
    async saveOrder(orderedPaths) {
        ...
        this.outputChannel.appendLine(`[Reordering] Saved order for ${cleaned.length} contract(s)`);
    }
    /**
     ...
     */
    async move(allContracts, fromPath, toPath) {
        if (fromPath === toPath) {
        ...
        this.outputChannel.appendLine(`[Reordering] Moved "${fromPath}" → before "${toPath}"`);
        return workingOrder;
    }
    /**
...
exports.ReorderingService = ReorderingService;
