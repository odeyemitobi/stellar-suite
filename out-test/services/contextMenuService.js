...
exports.ContractContextMenuService = void 0;
exports.registerCustomContextAction = registerCustomContextAction;
exports.resolveContextMenuActions = resolveContextMenuActions;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const _customActions = [];
function registerCustomContextAction(registration) {
    _customActions.push(registration);
    return new vscode.Disposable(() => {
        const idx = _customActions.indexOf(registration);
        if (idx !== -1) {
            _customActions.splice(idx, 1);
        }
    });
}
// ── Context detection ─────────────────────────────────────────
 ...
 */
function resolveContextMenuActions(req) {
...
// ── Action handlers ───────────────────────────────────────────
class ContractContextMenuService {
    ...
    async handleCopyContractId(contract) {
        if (!contract.contractId) {
    ...
    async handleOpenContractFolder(contract) {
    ...
    async handleViewDeploymentHistory(contract) {
        // Reads deployment history from workspace state (same key used by deployContract command)
        const history = this.context.workspaceState.get('stellarSuite.deploymentHistory', []);
        const contractHistory = history.filter((d) => d['contractName'] === contract.name);
        if (contractHistory.length === 0) {
            return { type: 'info', message: `No deployment history found for ${contract.name}.` };
        }
        const panel = vscode.window.createWebviewPanel('stellarSuite.deploymentHistory', `Deployment History – ${contract.name}`, vscode.ViewColumn.One, { enableScripts: false });
    ...
    async handleRename(contract) {
        ...
        const aliases = this.context.workspaceState.get('stellarSuite.contractAliases', {});
    ...
    async handleDuplicate(contract) {
        ...
        const newName = await vscode.window.showInputBox({
            ...
            validateInput: (v) => {
                ...
                if (fs.existsSync(dest)) {
                    return `Directory "${v.trim()}" already exists.`;
                }
                return undefined;
            },
        });
    ...
    async handleDelete(contract) {
        const answer = await vscode.window.showWarningMessage(`Remove "${contract.name}" from the workspace view?`, { modal: true, detail: 'This removes it from the Stellar Suite sidebar. The files will not be deleted.' }, 'Remove', 'Cancel');
        if (answer !== 'Remove') {
            return { type: 'info', message: 'Remove cancelled.' };
        }
        // Store hidden contracts list in workspace state
        const hidden = this.context.workspaceState.get('stellarSuite.hiddenContracts', []);
    ...
    async handlePin(contract) {
        const pinned = this.context.workspaceState.get('stellarSuite.pinnedContracts', []);
    ...
    async handleSetNetwork(contract) {
        ...
        const overrides = this.context.workspaceState.get('stellarSuite.contractNetworkOverrides', {});
...
exports.ContractContextMenuService = ContractContextMenuService;
// ── Helpers ───────────────────────────────────────────────────
...
function buildDeploymentHistoryHtml(contractName, history) {
