...
import {
  ...
  OperationProgressStatusBar,
} from "../ui/progressComponents";
      ...
      async (
        ...
        try {
          ...
          if (selectedContractPath) {
            ...
            if (fs.existsSync(selectedContractPath)) {
              ...
              if (stats.isFile() && selectedContractPath.endsWith(".wasm")) {
                ...
                deployFromWasm = true;
                outputChannel.appendLine(
                  `Using selected WASM file: ${wasmPath}`,
                );
                ...
                if (fs.existsSync(cargoToml)) {
                  ...
                  outputChannel.appendLine(
                    `Using selected contract directory: ${contractDir}`,
                  );
                } else {
                  ...
                  if (fs.existsSync(parentCargoToml)) {
                    ...
                    outputChannel.appendLine(
                      `Using parent contract directory: ${contractDir}`,
                    );
                  } else {
                    const wasmFiles = fs
                      .readdirSync(selectedContractPath)
                      .filter((f: string) => f.endsWith(".wasm"));
                    if (wasmFiles.length > 0) {
                      wasmPath = path.join(selectedContractPath, wasmFiles[0]);
                      deployFromWasm = true;
                      outputChannel.appendLine(
                        `Found WASM file in directory: ${wasmPath}`,
                      );
                    } else {
                      contractDir = selectedContractPath;
                      outputChannel.appendLine(
                        `Using selected directory as contract: ${contractDir}`,
                      );
                    }
                  }
                }
          ...
          if (!contractDir && !wasmPath) {
            ...
            outputChannel.appendLine(
              `Found ${contractDirs.length} contract directory(ies) in workspace`,
            );

            const wasmFiles = await WasmDetector.findWasmFiles();
            outputChannel.appendLine(
              `Found ${wasmFiles.length} WASM file(s) in workspace`,
            );
            ...
            if (contractDirs.length > 0) {
              if (contractDirs.length === 1) {
                ...
                outputChannel.appendLine(
                  `Using contract directory: ${contractDir}`,
                );
              } else {
                const fs = require("fs");
                const selected = await vscode.window.showQuickPick(
                  ...
                  {
                    placeHolder:
                      "Multiple contracts found. Select one to deploy:",
                  },
                );
                if (!selected) {
                  return;
                ...
                outputChannel.appendLine(
                  `Selected contract directory: ${contractDir}`,
                );
              }

              if (contractDir) {
                const expectedWasm =
                  WasmDetector.getExpectedWasmPath(contractDir);
                const fs = require("fs");
                if (expectedWasm && fs.existsSync(expectedWasm)) {
                  const useExisting = await vscode.window.showQuickPick(
                    [
                      {
                        label: "Deploy existing WASM",
                        value: "wasm",
                        detail: expectedWasm,
                      },
                    ...
                    {
                      placeHolder:
                        "WASM file found. Deploy existing or build first?",
                    },
                  );
              ...
              if (wasmFiles.length === 1) {
                ...
                const selected = await vscode.window.showQuickPick(
                  ...
                  {
                    placeHolder:
                      "Multiple WASM files found. Select one to deploy:",
                  },
                );
                if (!selected) {
                  return;
              ...
              if (contractDir) {
                outputChannel.appendLine(
                  `Found contract from active file: ${contractDir}`,
                );
              }
            }
          }
          ...
          if (!contractDir && !wasmPath) {
            const action = await vscode.window.showQuickPick(
              ...
              {
                placeHolder:
                  "No contract detected in workspace. How would you like to proceed?",
              },
            );
          ...
          const signingWorkflow = new DeploymentSigningWorkflowService(
            context,
            outputChannel,
          );
          const signingConfig = vscode.workspace.getConfiguration(
            "stellarSuite.signing",
          );
          ...
          if (!result && !deployableWasmPath) {
            vscode.window.showErrorMessage(
              "Build succeeded but no WASM output could be located.",
            );
            return;
          }
          ...
          if (!result && deployableWasmPath) {
            ...
            const signingPayload = await signingWorkflow
              .getSigningService()
            ...
            operation.report({
              percentage: 60,
              message: "Signing deployment transaction...",
            });
            signingResult = await signingWorkflow.run({
              ...
              requireValidatedSignature: signingConfig.get<boolean>(
                "requireValidatedSignature",
                true,
              ),
              enableSecureKeyStorage: signingConfig.get<boolean>(
            ...
            if (!signingResult.success) {
              outputChannel.appendLine(
                `❌ Signing failed: ${signingResult.error}`,
              );
              operation.fail(signingResult.error || "Signing failed");
              vscode.window.showErrorMessage(
                `Deployment signing failed: ${signingResult.error}`,
              );
              return;
            }
            ...
            if (signingResult.publicKey) {
              outputChannel.appendLine(
                `[Signing] Signer: ${signingResult.publicKey}`,
              );
            }
            outputChannel.appendLine(
              `[Signing] Payload hash: ${signingResult.payloadHash}`,
            );
          ...
          if (result.success) {
            ...
            if (result.contractId) {
              ...
              const effectiveContractDir =
                contractRootDir || resolvedContractDir;

              let contractNameForRecord = "unknown";
              if (effectiveContractDir) {
                try {
                  const fs = require("fs");
                  const cargoPath = path.join(
                    effectiveContractDir,
                    "Cargo.toml",
                  );
              ...
              const deployedContracts = context.workspaceState.get<
              ...
              try {
                ...
                if (tracker && effectiveContractDir) {
                  const localVersion =
                    tracker.getLocalVersion(effectiveContractDir);
                  if (localVersion) {
            ...
            operation.fail(
              result.errorSummary ?? result.error ?? "Deployment failed",
              notificationMessage,
            );
