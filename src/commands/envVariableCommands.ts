...
function formatVariableList(
  ...
  if (entries.length === 0) {
    ...
    for (const [key, value] of entries) {
      const isSensitive = profile?.variables.find(
        (v) => v.name === key,
      )?.sensitive;
    ...
    if (pick.label === "$(add) Add Variable") {
      const rawName = await vscode.window.showInputBox({
        ...
        validateInput: (value: string) => {
          ...
          const nameResult = sanitizer.sanitizeEnvVarName(value, {
            field: "name",
          });
          if (!nameResult.valid) {
            return nameResult.errors[0];
          }
      ...
      const nameResult = sanitizer.sanitizeEnvVarName(rawName, {
        field: "name",
      });
      if (!nameResult.valid) {
        continue;
      ...
      const valueResult = sanitizer.sanitizeEnvVarValue(rawValue, {
        field: "value",
      });
        ...
        [
          {
            ...
            description: "Change the variable value",
          },
      ...
      if (action.label === "$(trash) Remove") {
        ...
        const newValueResult = sanitizer.sanitizeEnvVarValue(rawNewValue, {
          field: "value",
        });
        variables[varIndex] = {
          ...variables[varIndex],
          value: newValueResult.sanitizedValue,
        };
      }
    }
  }
      ...
      while (true) {
        ...
        const actions: vscode.QuickPickItem[] = [
          ...
          },
          {
            ...
            description: "Create a new env variable profile",
          },
        ];
          ...
          actions.push(
            {
              ...
              description: "Activate a different profile",
            },
            {
        ...
        actions.push(
          { label: "", kind: vscode.QuickPickItemKind.Separator },
          {
            label: "$(cloud-download) Import Profiles",
            description: "Import from JSON file",
          },
          {
            label: "$(cloud-upload) Export Profiles",
        ...
        if (action.label.includes("Create Profile")) {
          ...
          try {
            const profile = await envService.createProfile(
              name,
              variables,
              description,
            );
            ...
            if (activate === "Activate") {
              ...
              vscode.window.showInformationMessage(
                `Profile "${profile.name}" is now active.`,
              );
            }
          } catch (err) {
        ...
        if (action.label.includes("Switch Profile")) {
          ...
          if (pick.label.includes("None")) {
            ...
            vscode.window.showInformationMessage(
              "Environment variable profile deactivated.",
            );
          } else {
            ...
            if (selected) {
              ...
              vscode.window.showInformationMessage(
                `Profile "${selected.name}" is now active.`,
              );
            }
          }
          continue;
        ...
        if (action.label.includes("Export")) {
          ...
          try {
            ...
            vscode.window.showInformationMessage(
              "Environment variable profiles exported.",
            );
          } catch (err) {
            vscode.window.showErrorMessage(
